...
download_git_output_processor() {
  url="https://raw.fastgit.org/${ZINIT_REPO}/${ZINIT_COMMIT:-${ZINIT_BRANCH}}/share/git-process-output.zsh"
...
zinit_install() {
    ...
    "${COLOR_BOLD_MAGENTA}${ZINIT_INSTALL_DIR}"
  {
    command git clone --progress --branch "$ZINIT_BRANCH" \
      "https://hub.fastgit.xyz/${ZINIT_REPO}" \
...
if [[ ! -f ${zinit_home_escaped}/${ZINIT_REPO_DIR_NAME}/zinit.zsh ]]; then
    ...
    command git clone https://hub.fastgit.xyz/${ZINIT_REPO} "${zinit_home_escaped}/${ZINIT_REPO_DIR_NAME}" && \\
...
- The ${COLOR_BOLD_WHITE_ON_BLACK}README${COLOR_RESET} section on the ice-modifiers:
    🧊 https://hub.fastgit.xyz/${ZINIT_REPO}#ice-modifiers
...
- 🔖 Or on GitHub: https://hub.fastgit.xyz/zdharma-continuum
EOF
}
